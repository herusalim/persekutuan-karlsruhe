...
public class SheetsDataProvider {
            ...
            if (rangeJadwalPelayanan.equals(rawData.getRange())) {
                ...
                for (int i = 1; i < rows.size(); i++) {
                    ...
                    if (row.size() > 0) {
                        ...
                        if (fieldTanggal instanceof BigDecimal || (fieldTanggal instanceof String && !StringUtils.isEmpty((String) fieldTanggal))) {
                            LOG.info("Mencoba menambahkan jadwal pelayanan untuk baris: " + row);
                            // valid entry
                            ...
                            LOG.info("Jadwal pelayanan yang berhasil di extract: " + jadwalPelayanan);
                            ...
                            if (!futureOnly
                                ...
                                if ((!futureOnly || jadwalPelayanan.getTanggal().after(now.getTime()))
                                    ...
                                    LOG.info("Menambahkan jadwal berikut ke daftar: " + jadwalPelayanan);
                                    daftarJadwalPelayanan.add(jadwalPelayanan);
                                } else {
                                    LOG.info("Entri " + row + " diabaikan, karena bahan renungan masih kosong");
                                }
                            } else {
                                LOG.warning("Jadwal dilompati karena sudah di masa lalu. Futureonly: " + futureOnly + "\t" + toDate(CalendarUtil.getJadwalTimeValue(jadwalPelayanan)) + "\t" + toDate(now.getTimeInMillis()));
                            }
                        } else if (!StringUtils.isEmpty(fieldTanggal)) { // Abaikan empty string
    ...
    private JadwalPelayanan createJadwalPelayanan(List<Object> row, List<Object> headerRow, int indexMateriRenungan,
        ...
        Object dateField = row.get(0);
        if (dateField instanceof BigDecimal) {
            jadwalPelayanan = new JadwalPelayanan((BigDecimal) dateField);
        } else if (dateField instanceof String) {
            jadwalPelayanan = new JadwalPelayanan((String) dateField);
        } else {
            throw new IllegalStateException("Invalid date format should have been filtered before");
        }
